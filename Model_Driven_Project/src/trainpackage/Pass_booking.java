...
class Pass_booking extends Booking{
	int seat_no;
	String username,meal_type;
	...
	public static int cancel_booking(int train_no,int seat_no,ArrayList<Pass_booking> pass_book_list,Pass_train_seat_details pass_trains[]){
		for (int i=0;i<pass_book_list.size();i++){
			if(pass_book_list.get(i).train_no== train_no && pass_book_list.get(i).seat_no==seat_no){
				...
				System.out.println("Please enter a valid ticket details");
